...
<?php 
    ...
    // $id = $_SESSION['id'];
     ...
     if (isset($_POST['update'])) {
        ...
        $updatequery = "UPDATE `project` SET `DATE IN`= '$Udate', `BENEFICIARY`= '$Ubeneficiary', `AMOUNT`= '$Uamount',`treasury no`= '$Utnum',`voucher no`= '$Uvnum',`are receipts ok?`= '$Uare_receipts_ok',`is narration ok?`= '$Uis_narration_ok',`is account changed?`= '$Uis_account_changed',`Batch no`= '$Ubatch_no', `transaction type`= '$Utransaction_type',`payment date`= '$Upayment_date', `are signatures ok?`= '$Uare_signatures_ok',`Are assets included?`= '$Uare_assets_included',`is cert. of honour ok?`= '$Uis_cert_honour',`cacu list`= '$Ucacu_list', `int audit list`= '$Uint_audit_list',`fixed asset list`= '$Ufixed_asset_list', `final acct list`= '$Ufinal_acct_list', `is voucher paid in instalments?`= '$Uvoucher_instalment',`is voucher fully paid?`= '$Uvoucher_fully_paid',`voucher payment outstanding`= '$Uvoucher_outstanding',`voucher payment paid in full`= '$Uvoucher_payment_full',`voucher to copy list`= '$Uvoucher_copy',`Are receipts stamped?`= '$Uare_receipts_stamped',`is voucher retired?`= '$Uvoucher_retired' WHERE id = '$id'";
